...
const addChat = async (selectedUser) => {
  try {
    ...
    if (existingMessageId) {
      ...
      await Promise.all([
        setDoc(currentChatRef, { chatData: updatedCurrentChatData }, { merge: true }),
        setDoc(receiverChatRef, { chatData: updatedReceiverChatData }, { merge: true }),
      ]);
            ...
            .map((user) => {
              ...
              >
                ...
                <div>
                  ...
                  <span>{chat?.lastMessage || user.bio || "Available to chat"}</span>
                </div>
              </div>
              );
            })}
